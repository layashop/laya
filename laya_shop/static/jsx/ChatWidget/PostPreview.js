...
const PostPreview = ({data, ownMessage}) => {
    return <>{data.map((item) => {
        return (<a href={`/${item.business.slug}/${item.id}`} target="_blank" className="w-full flex items-center mb-1 rounded-md hover:bg-gray-200 transition duration-100" key={item.id}>
